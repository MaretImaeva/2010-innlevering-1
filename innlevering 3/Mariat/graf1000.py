...
def les_skue():
    ...
    for line in data:
        ...
        alt_dict[deler[0]] = {"navn": deler[1], "kant": []}
        filmer = []
        for film in deler[2:]:
            filmer.append(film)
...
for film_obj, actors in film_dict.values():
    for a, b in combinations(actors, 2):
        alt_dict[a.id]["kant"].append((b.id, film_obj.film_navn, film_obj.film_id, film_obj.vekt))
...
def antall_kanter():
    ...
    for val in alt_dict.values():
        for nabo, tittel, filmID, vekt in val["kant"]:
            kanter += 1
    return "Antall kanter:", kanter

def finn_node(node_id):
    return alt_dict[node_id]
...
def hent_vekt(node_id):
    nabo, tittel, filmID, vekt = alt_dict(node_id)["kant"]
    return vekt
